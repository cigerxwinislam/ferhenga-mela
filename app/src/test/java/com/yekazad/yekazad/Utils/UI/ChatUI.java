// Bismillahirrahmannirrahim
// Elhamdulillahirabbulalemin
// Esselatu ve sselamu alâ Resulillah

package com.yekazad.yekazad.Utils.UI;
/**
 * Created by Mohamed El Sayed
 */
public class ChatUI {
    public static final String BUNDLE_RECIPIENT = "BUNDLE_RECIPIENT";
    // target class to be called in listeners (such as OnProfileClickListener)
    public static final String BUNDLE_MESSAGE = "BUNDLE_MESSAGE";
    public static final String BUNDLE_CHAT_GROUP = "BUNDLE_CHAT_GROUP";
    public static final String BUNDLE_GROUP_ID = "BUNDLE_GROUP_ID";
    public static final String BUNDLE_GROUP_USER = "BUNDLE_GROUP_USER";
    public static final String BUNDLE_CHANNEL_TYPE = "BUNDLE_CHANNEL_TYPE";

    public static final String BUNDLE_SIGNED_UP_USER_EMAIL = "BUNDLE_SIGNED_UP_USER_EMAIL";
    public static final String BUNDLE_SIGNED_UP_USER_PASSWORD = "BUNDLE_SIGNED_UP_USER_PASSWORD";
    //
    public static final String BUNDLE_CHAT_USERNAME = "BUNDLE_CHAT_USERNAME";
    public static final String BUNDLE_CHAT_PROFILE_PIC = "BUNDLE_CHAT_PROFILE_PIC";
    public static final String BUNDLE_CHAT_STATUS = "BUNDLE_CHAT_STATUS";
    public static final String BUNDLE_CHAT_NAME = "BUNDLE_CHAT_NAME";
    public static final String BUNDLE_CHAT_ID = "BUNDLE_CHAT_ID";
    public static final String BUNDLE_IMAGE_EDIT = "BUNDLE_IMAGE_EDIT";
    public static final String BUNDLE_CURRENT_USERNAME = "BUNDLE_CURRENT_USERNAME";

    //
    // request constants
    public static final int REQUEST_CODE_CREATE_GROUP = 100;
    public static final int REQUEST_CODE_EDITED_IMAGE= 101;
    public static final int REQUEST_CODE_SIGNUP_ACTIVITY = 200;
}
